-- Adab.ng lister portal — fully idempotent schema
-- Safe on fresh projects AND upgrades from older static-site properties tables.
-- Run entire file in Supabase SQL Editor. Safe to re-run after partial failures.

create extension if not exists "pgcrypto";

-- ---------------------------------------------------------------------------
-- Profiles (must exist before properties.owner_id FK)
-- ---------------------------------------------------------------------------
...
-- Normalize legacy column types and values before CHECK constraints
do $$
begin
  if exists (
...
update public.properties set status = coalesce(status, 'published') where status is null;
...
where status is not null
  and status not in ('draft', 'pending_review', 'published', 'unavailable', 'rejected');

update public.properties
set type = 'sale'
where type is not null and lower(btrim(type)) in ('sale', 'buy', 'purchase');

update public.properties
set type = 'rent'
where type is not null and lower(btrim(type)) in ('rent', 'rental', 'lease');
...
-- ---------------------------------------------------------------------------
-- Promote your first admin (replace email before running)
-- ---------------------------------------------------------------------------
-- update public.profiles set role = 'admin' where email = 'hello@adab.ng';
